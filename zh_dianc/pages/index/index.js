...
Page({
    ...
    onLoad: function(a) {
        var e = this, t = wx.getStorageSync("bqxx");
                        ...
                        console.log(t), "" == t ? wx.showToast({
                            ...
                            success: function(a) {
                                console.log(a), e.setData({
                                    userinfo: a.data
                                }), wx.setStorageSync("users", a.data), app.util.request({
                                    url: "entry/wxapp/New",
                                    cachetime: "0",
                                    data: {
                                        user_id: a.data.id,
        ...
        var s = Number(a.type);
        1 == s ? console.log("用户选择的是店内点餐") : console.log("用户选择的是外卖点餐"), e.setData({
            types: s
        });
        ...
        })), a.showview, a.showView, e.reload();
    },
    reload: function(a) {
    ...
    decreaseShopCart: function(a) {
        ...
        var e = a.currentTarget.dataset.itemIndex, s = a.currentTarget.dataset.parentindex, o = a.currentTarget.dataset.gwcindex, d = a.currentTarget.dataset.ggindex;
        if (null != d) {
            if (0 == this.data.dishes[s].goods[e].one || 0 == this.data.dishes[s].goods[e].gg[d].num) return;
            this.data.dishes[s].goods[e].one--, this.data.dishes[s].goods[e].gg[d].num--;
            var i = "a" + e + "b" + s + "c" + this.data.dishes[s].goods[e].gg[d].id;
            if (2 == this.data.types) var n = this.data.dishes[s].goods[e].gg[d].cost; else n = this.data.dishes[s].goods[e].gg[d].cost;
            var r = this.data.dishes[s].goods[e].box_fee;
            ...
            if (console.log(d), 0 == t.data.dishes[s].goods[e].one) return;
    ...
    pingjia: function (t) {
      console.log("商家ID"), wx.navigateTo({
        url: "../comment/comment?seller_id=" +this.data.seller_id
      });
    },
    decreaseCart: function(a) {
        ...
        console.log(u, p), console.log(p, h.data.dishes[p].goods[u].id), wx.showLoading({
            title: "加载中",
            mask: !0
        }), app.util.request({
            ...
            success: function(a) {
                if (console.log(a), 0 != a.data.length) wx.showModal({
                    ...
                    if (0 == h.data.dishes[p].goods[u].one) return;
    ...
    addCart: function(a) {
        ...
        this.setData({
            zindex: u,
            findex: p
        }), console.log(p, h.data.dishes[p].goods[u].id), wx.showLoading({
            title: "加载中",
            mask: !0
        }), app.util.request({
            url: "entry/wxapp/DishesGg",
    ...
    calTotalPrice: function() {
        for (var a = this.data.carArray, t = 0, e = 0, s = 0, o = 0; o < a.length; o++) 0 < a[o].num && (2 == this.data.types ? (t += a[o].money * a[o].num + a[o].box_fee * a[o].num, 
    ...
    pay: function(a) {
        ...
        if (console.log(t), "" != t.img && "" != t.name) if (2 == this.data.types) {
            s = this.data.shop_cart;
            if (console.log(this.data.shop_cart), console.log(this.jcgwc(s)), console.log(this.data), 
